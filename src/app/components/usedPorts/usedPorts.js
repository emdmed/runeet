...
const UsedPorts = () => {
    ...
    return <div className="flex gap-1 overflow-auto w-full py-4 items-center">
        ...
        <Button onClick={() => setIsCollapsed(prev => !prev)} className="text-foreground border-destructive h-[20px]" variant="outline">{usedPorts?.length || 0} used ports {isCollapsed ? <ChevronRight /> : <ChevronLeft />}</Button>
